...
?><div class="location"><?
	...
	?><form action="<?=$arResult['ACTION_URL']?>" method="POST" id="inheadlocform"><?
		...
		$frame->setBrowserStorage(true);
			...
			?><span><?=GetMessage('RSGOPRO_QUESTION_1')?>: </span><a class="fancyajax fancybox.ajax big" href="<?=SITE_DIR?>include/popup/mycity/" title="<?=GetMessage('RSGOPRO_QUESTION_2')?>"><?=$arResult['LOCATION']['CITY_NAME']?><i class="icon pngicons"></i></a><?
		$frame->beginStub();
