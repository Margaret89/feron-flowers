...
if(is_array($arResult['ITEMS']) && count($arResult['ITEMS'])>0) {
	...
	if($arParams['IS_AJAXPAGES']=="Y") {
		$this->SetViewTarget("catalognames");
						...
						foreach($arResult['ITEMS'] as $key1 => $arItem) {
							...
							?><tr class="js-name<?=$arItem['ID']?><?if( ($inc+1)%2==0 ):?> even<?endif;?>" data-detail="<?=$arItem['DETAIL_PAGE_URL']?>" data-elementid="<?=$arItem['ID']?>"><?
								?><td class="free"><?
									?><span<?
										if( 
	...
	if($arParams['IS_AJAXPAGES']=="Y") {
		...
		foreach($arResult['ITEMS'] as $key1 => $arItem) {
			...
			$HAVE_OFFERS = (is_array($arItem['OFFERS']) && count($arItem['OFFERS'])>0) ? true : false;
			if($HAVE_OFFERS) { $PRODUCT = &$arItem['OFFERS'][0]; } else { $PRODUCT = &$arItem; }
			?><tr class="js-element js-elementid<?=$arItem['ID']?> <?if($HAVE_OFFERS):?>offers<?else:?>simple<?endif;?><?if( ($inc+1)%2==0 ):?> even<?endif;?>" data-detail="<?=$arItem['DETAIL_PAGE_URL']?>" data-elementid="<?=$arItem['ID']?>" id="<?=$this->GetEditAreaId($arItem["ID"]);?>"><?
				?><td class="free"><?
					?><span<?
						if( 
							isset($arItem['DAYSARTICLE2']) ||
