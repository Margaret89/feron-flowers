	...
	if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "N") {
		...
		if($arResult["USER_VALS"]["CONFIRM_ORDER"] == "Y" || $arResult["NEED_REDIRECT"] == "Y") {
			...
			function checkLocation() {
				if (!isItLocationed) {					
					isItLocationed = true;
					console.warn('check location!');
					if( ($('.line.f_location').find('.search-suggest').val()!='' && parseInt($('.line.f_location').find('input[type="hidden"]').val())>0) || $(".line.f_location select option:selected").val()!='' ) {
						setTimeout(submitForm,200);
					}
				}
			}
			$(document).on('click', '.persontypes input[type=radio]', function() {
				isItLocationed = false;
			});
			...
			<?if($_POST["is_ajax_post"] != "Y") {
				?>
					...
					<div class="bx_ordercart_order_pay_center"><a class="btn btn1 orderbtn" href="javascript:void();" onclick="submitForm('Y'); return false;" class="checkout"><?=GetMessage("SOA_TEMPL_BUTTON")?></a></div>
				</form>
				<?
