...
if(is_array($arResult['ITEMS']) && count($arResult['ITEMS'])>0) {
	...
	foreach($arResult['ITEMS'] as $key1 => $arItem) {
		...
		?><div class="js-element js-elementid<?=$arItem['ID']?> <?if($HAVE_OFFERS):?>offers<?else:?>simple<?endif;?><?
			...
			?>id="<?=$this->GetEditAreaId($arItem["ID"]);?>" data-detail="<?=$arItem['DETAIL_PAGE_URL']?>"><?
			?><div class="inner"><?
				?><div class="padd"><?
									...
									?><div class="timer <?if(isset($arTimer['DINAMICA_EX'])):?>da2<?else:?>qb<?endif;?> js-timer_id<?=$arTimer['ELEMENT_ID']?> clearfix" style="display:<?
										...
										?><div class="intimer clearfix"  data-dateto="<?=$arTimer[$KY]['DATE_TO']?>" data-autoreuse="<?=$arTimer['AUTO_RENEWAL'];?>"><?
											...
											?><div class="val"><div class="value result-minute"><?
												...
												?><div class="dvoet" <?echo ($arTimer[$KY]['DAYS']>0 ? 'style="display:none;"' : '""');?>>:</div><?
												?><div class="val" <?echo ($arTimer[$KY]['DAYS']>0 ? 'style="display:none;"' : '""');?>><div class="value result-second"><?
					...
					?><div class="popup padd"><?
						...
						?><div class="favorishare clearfix"><?
							if($arParams['USE_FAVORITE']=='Y' || $arParams['USE_SHARE']=='Y')
							{
								...
								{
									?><div class="share"><?
										?><span class="b-share"><a class="fancyajax fancybox.ajax email2friend b-share__handle b-share__link b-share-btn__vkontakte" href="<?=SITE_DIR?>include/popup/email2friend/?link=<?=CUtil::JSUrlEscape('http://'.$_SERVER['HTTP_HOST'].$arItem['DETAIL_PAGE_URL'])?>" title="<?=GetMessage('EMAIL2FRIEND')?>"><i class="b-share-icon icon pngicons"></i></a></span><?
